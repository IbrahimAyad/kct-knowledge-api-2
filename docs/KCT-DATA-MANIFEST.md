# KCT Menswear — Knowledge Base Data Manifest
> **Purpose**: This document gives the KB API everything it needs to make recommendations, validate outfits, and power upsells using *real KCT data* — not generic menswear guesses.
>
...
**Table**: `bundle_orders`
**What it powers**: "Customers who bought this suit most often paired it with X shirt/tie" — real pairing data, not guesses.
...
**What it powers**: Which bundles/combos get abandoned → signals bad pairings or price mismatches.
...
**What it powers**: Users who liked/disliked/purchased AI suggestions → trains better recs.
...
**What it powers**: Which products sell when → validates seasonal micro-patterns with YOUR data.

| Field | Use |
|-------|-----|
| `product_id` + `date` | Time-series product demand |
| `views` → `add_to_cart` → `purchases` | Full funnel per product |
...
**What it powers**: Real wedding types/months/seasons your customers choose → better occasion recs.
...
- "Color didn't match in person" → validates color science
- "Didn't fit the occasion" → validates venue/event logic
- "Quality not as expected" → validates price tier pairing
...
**KCT's competitive advantage.** We stock 69 tie/bowtie colors — far more than most competitors. The KB should leverage this to always find a perfect color match.

Colors with matching suspender+bowtie sets marked ✅:

Apple Red, Aqua, Baby Blue, Banana Yellow, Beige, **Black ✅**, Blush, Burgundy, Burnt Orange, Canary, Carolina Blue, Champagne, Charcoal, Chianti, Chocolate Brown, Cinnamon, Cobalt, Coral, **Dark Grey ✅**, Dark Navy, Dark Olive, Dark Red, Dark Silver, Deep Purple, Denim Blue, Dusty Pink, Dusty Rose, Dusty Sage, Emerald Green, Forest Green, French Blue, French Rose, Fuchsia, **Gold ✅**, Ivory, Lavender, Lettuce Green, **Light Blue ✅**, Light Blush, Light Lilac, Light Pink, Lilac, Lime, Magenta, Mauve, Medium Brown, Medium Orange, Medium Purple, Mermaid Green, Mint Green, Navy Blue, Nutmeg, Olive Green, Orange, Pastel Green, Pastel Purple, Peach, **Pink ✅**, Plum, Powder Blue, **Purple ✅**, Rose Gold, **Royal Blue ✅**, Rust, Salmon, Sapphire Blue, **Silver ✅**, Taupe, Teal, Tiffany Blue, **True Red ✅**, Turquoise, **White ✅**, **Wine ✅**

**KB Rules for Tie Matching**:
1. Always match to a tie color we carry — with 69 colors, there's almost always a match
...
4. Prom dates: same logic — match to their date's dress color
...
|------|---------------|-------------|---------------|-------------|
| **Entry** | $119–$179 | $19–$34 | $24.99 | $49–$79 |
| **Mid** | $179–$259 | $34–$49 | $24.99–$44.99 | $79–$109 |
| **Premium** | $259–$400 | $49–$69 | $39.99–$49.99 | $109–$129 |
...
1. **Color Inventory** (Part 3.2) — Load immediately. Stop recommending colors we don't carry.
2. **Price Tier Rules** (Part 3.5) — Load immediately. Stop mismatched price recommendations.
3. **Product Tags** (Part 3.6) — Use for occasion/venue matching logic.
4. **Tie Color Palette** (Part 3.3) — Enable bridesmaid/prom dress color matching.
5. **Category Catalog** (Part 3.4) — Reference for what product types exist.
6. **Bundle History** (Part 1.1) — Ingest as orders grow. Replace algorithmic pairing with real data.
7. **Seasonal Calendar** (Part 3.7) — Time-weight recommendations.
...
### 4.3 Critical Rules
1. **Never recommend a color we don't stock** — check against Part 3.2
2. **Never pair across 2+ price tiers** — check against Part 3.5
3. **Weight occasion tags** — a "prom" query should surface prom-tagged products first
4. **Respect inventory tiers** — push Core/Strong colors, use Limited/Rare only when context-matched
5. **Use real purchase data over algorithmic guesses** — when `bundle_orders` has data for a combo, prefer it

### 4.4 Data Refresh Schedule

| Data | Refresh Frequency | Method |
|------|-------------------|--------|
| Product catalog | Daily | Supabase → KB sync |
| Color inventory | Daily | Tag aggregation query |
| Bundle history | Real-time | Webhook on `bundle_orders` insert |
| Cart abandonment | Daily batch | Supabase → KB analytics |
...
These content files contain domain expertise that the KB should eventually ingest. They're NOT in this repo — they're in KCT's content/marketing repos.

### 6.1 Content Inventory

| Category | Files | What the KB learns |
|----------|-------|--------------------|
| **Seasonal Intelligence** | 12 monthly calendars (`blog-calendar/2026/`) | What to recommend WHEN — month-by-month content themes, product pushes, promotional hooks |
...
- Wedding color combos by venue type → extends `color-relationships.json`
- Prom aura-to-color mappings → new `prom-aura-colors.json`
- Monthly product push priorities → extends seasonal rules engine
- Michigan venue/school lists → extends venue intelligence with local data
...
| Loyalty Triggers | 📋 Available, not wired |
