...
class SystemHealthService {
  ...
  async getTrendingHealth(): Promise<{
    ...
    try {
      ...
      return {
        ...
        cache_performance: {
          hit_rate: 0.85, // Mock data - cache service doesn't provide hitRate
          average_latency: 50, // Mock data
          memory_usage: typeof cacheHealth.memoryUsage === 'string' ? 0 : (cacheHealth.memoryUsage || 0)
        },
        analysis_performance: {
          average_processing_time: analysisHealth.processing_time,
          data_freshness_minutes: analysisHealth.data_freshness,
          predictions_generated: (analysisHealth as any).predictions_count || 0
        },
        data_quality: {
  ...
  private async checkCacheHealth(): Promise<ServiceHealth['cache']> {
    try {
      ...
      return {
        ...
        latency,
        hit_rate: 0.85, // Mock data - cache service doesn't provide hitRate
        memory_usage: typeof health.memoryUsage === 'string' ? 0 : (health.memoryUsage || 0),
        connections: (health as any).connections || 0
      };
    } catch (error) {
  ...
  private startMetricsCollection(): void {
    ...
    setInterval(() => {
      ...
      logger.debug('Cleaning old metrics', { 
        metadata: { metricsCount: this.metrics.requests.size }
      });
    }, 5 * 60 * 1000);
  }
