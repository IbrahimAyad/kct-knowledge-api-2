...
class TrendingAnalysisService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.trendingData = await dataLoader.loadIntelligenceData('trending-now');
      this.conversionData = await dataLoader.loadIntelligenceData('conversion-rates');
      this.demographicData = await dataLoader.loadIntelligenceData('age-demographics');
      this.seasonalData = await dataLoader.loadIntelligenceData('seasonal-champions');
  ...
  async getTrendingCombinations(
    ...
    try {
      ...
      logger.info(`Analyzing trending combinations with filters:`, filters);
      ...
      await cacheService.set(cacheKey, sortedTrending, 900);
  ...
  async getTrendingColors(
    ...
    try {
      ...
      await cacheService.set(cacheKey, colorTrends, 1800);
  ...
  async getSeasonalTrends(season?: string): Promise<SeasonalTrend[]> {
    ...
    try {
      ...
      // Cache for 6 hours
      await cacheService.set(cacheKey, seasonalTrends, 21600);
  ...
  async getVenueTrends(venueType?: string): Promise<VenueTrend[]> {
    ...
    try {
      ...
      // Cache for 2 hours
      await cacheService.set(cacheKey, venueTrends, 7200);
  ...
  async getDemographicTrends(
    ...
    try {
      ...
      logger.info('Analyzing demographic trends with filters:', filters);
      
      const demographicTrends = await this.processDemographicTrends(filters);
      
      // Cache for 4 hours
      await cacheService.set(cacheKey, demographicTrends, 14400);
   ...
   */
  async getTrendPredictions(timeframe: '7d' | '30d' | '90d' = '30d'): Promise<TrendPrediction> {
    if (!this.initialized) await this.initialize();
  ...
  async getTrendingAlerts(): Promise<{
    ...
    try {
      ...
      await cacheService.set(cacheKey, alerts, 600);
